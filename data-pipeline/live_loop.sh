...
# quotes-fast.json en el branch `data-fast`. Cerrado: duerme 10 min y sale (la cadena reinicia).
# Requiere env GH_PUSH_URL (con token) para el push.
set -u
...
PUB="$ROOT/_pub"
mkdir -p "$PUB" && (cd "$PUB" && git init -q && git checkout -q -b data-fast)
...
while [ $SECONDS -lt $END ]; do
  ...
  if [ -f "$ROOT/data-pipeline/output/quotes-fast.json" ]; then
    ...
    (cd "$PUB" && git add quotes-fast.json && git commit -q -m "fast $(date -u +%H%M%S)" && git push -q -f "$GH_PUSH_URL" data-fast) && echo "ciclo $n publicado ($(date -u +%H:%M:%S))"
  else
